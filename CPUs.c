...
void* RRcpu(void* param) {
   int threadNum = ((CpuParams*) param)->threadNumber;
    SharedVars* svars = ((CpuParams*) param)->svars;
    int quantamConsumed = 0; //Tracks the time quantam consumed by the current process
    ...
    while (1) {
        ...
        //The process consumed its allowed quantam
        else if(quantamConsumed == svars->quantum) {
            quantamConsumed = 0;
            // Lock readyQ before inspecting or modifying it — another CPU
            // thread (or main inserting a new arrival) could touch it right now.
            pthread_mutex_lock(&(svars->readyQLock));
            ...
            //Run the first process found in the queue
            p = qRemove(&(svars->readyQ), 0);
            if (p == NULL) {
                // readyQ was empty — CPU stays idle this tick.
                printf("No process to schedule\n");
        ...
        if (p != NULL) {
            ...
            if (p->burstRemaining == 0) {
                quantamConsumed = 0; //Set quantam consumed to zero when the process terminates
                // Process is done — move it to finishedQ so main can
                // compute and print wait-time statistics at simulation end.
                pthread_mutex_lock(&(svars->finishedQLock));
                qInsert(&(svars->finishedQ), p);
...
void* SRTFcpu(void* param) {
    ...
    while (1) {
        ...
        // SRTF is pre-emptive. The CPU chooses a new process to run when it's idle, or 
        // if another process arrives with a smaller SRTF
        if(p == NULL) {
            // Lock readyQ before inspecting or modifying it — another CPU
            // thread (or main inserting a new arrival) could touch it right now.
            pthread_mutex_lock(&(svars->readyQLock));
...
void* PPcpu(void* param) {
   int threadNum = ((CpuParams*) param)->threadNumber;
    ...
    while (1) {
        ...
        // ppcpu is pre-emptive. The CPU chooses a new process to run when it's idle, or 
        // if another process arrives with a higher priority
        if(p == NULL) {
            // Lock readyQ before inspecting or modifying it — another CPU
            // thread (or main inserting a new arrival) could touch it right now.
            pthread_mutex_lock(&(svars->readyQLock));
